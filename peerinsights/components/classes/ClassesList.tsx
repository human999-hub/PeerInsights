...
export default function ClassesList({ instructorEmail }: { instructorEmail: string }) {
  const { data, isLoading, isError, error } = useClassesByInstructor(instructorEmail);
  const [selected, setSelected] = React.useState<CreatedClass | null>(null);

  if (isLoading) {
    return <div className="max-w-5xl mx-auto p-6 surface-card rounded-xl">Loading classes…</div>;
  }
  if (isError) {
    return (
      <div className="max-w-5xl mx-auto p-6 surface-card rounded-xl">
        <p className="text-red-700 font-medium">Couldn’t load classes: {error?.message}</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="max-w-5xl mx-auto surface-card rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {data.map((cls) => (
                <tr key={cls._id} className="border-t border-black/10 hover:bg-white/40">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex gap-3 justify-end">
                      ...
                      <Link className="underline hover:no-underline" href={`/classes/${cls._id}/edit`}>
                        Edit
                      </Link>
                      <Link
                        className="underline hover:no-underline"
      ...
      {selected && (
        <ClassDetailsModal cls={selected} open={!!selected} onClose={() => setSelected(null)} />
      )}
    </>
  );
}
