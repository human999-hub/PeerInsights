...
//   }
// }
//  app/api/responses/instructor-summary/route.ts
import { NextResponse } from "next/server";
import { connectDB } from "@/lib/mongodb";
...
export async function GET(req: Request) {
  try {
    ...
    // 9) Load ALL questions once (so we can attach qid/title/description everywhere)
    const questions = await Question.find({})
      .select({ qid: 1, title: 1, description: 1 }) // ✅ only these
      .lean();
            ...
            return {
              ...
              responses: (responsesBySubmission.get(submissionId) || []).map(
                (r: any) => ({
                  ...
                  from_student_id: String(r.from_student_id),
                  to_student_id: String(r.to_student_id),
                  question: qInfo(r.question_id), // ✅ add details
                  rating: r.rating,
                })
              ),
              ...
              comments: (commentsBySubmission.get(submissionId) || []).map(
                (c: any) => ({
                  ...
                  question: qInfo(c.question_id), // ✅ add details
                  comment_text: c.comment_text,
                })
              ),
              ...
              praises: (praisesBySubmission.get(submissionId) || []).map(
                (p: any) => ({
                  ...
                  question: p.question_id ? qInfo(p.question_id) : null, // ✅ add details if present
                  praise_text: p.praise_text,
                })
              ),
            };
